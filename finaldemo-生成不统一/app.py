...
def build_tree_structure(knowledge_edges, topology_id, content: str, max_nodes: int = 0):
    ...
    if not root_nodes:
        # 如果没有入度为0的节点，选择入度最小的节点作为根节点
        min_in_degree = min(in_degree.values()) if in_degree else 0
        root_nodes = [node_id for node_id, degree in in_degree.items() if degree == min_in_degree]
    
    # 如果还是没有根节点，选择出度最大的节点
    if not root_nodes:
        max_out_degree = max(out_degree.values()) if out_degree else 0
        root_nodes = [node_id for node_id, degree in out_degree.items() if degree == max_out_degree]
    
    # 如果还是没有根节点，选择第一个节点
    if not root_nodes and nodes:
        root_nodes = [next(iter(nodes.keys()))]
    
    # 计算节点层级（从根节点开始的深度）
    def calculate_level(node_id, current_level=0):
        if nodes[node_id]["level"] < current_level:
    ...
    for node_id in nodes:
        # 连接数（入度+出度）
        connection_count = in_degree[node_id] + out_degree[node_id]
        # 层级因子（层级越低越重要）
